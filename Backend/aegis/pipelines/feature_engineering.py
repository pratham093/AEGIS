"""builds causal features per asset from the universe dataset."""
...
# pandas throws too many future/deprecation warnings for rolling ops
warnings.filterwarnings("ignore")

# resolve paths relative to Backend/ regardless of where we run from
...
def compute_log_returns(series: pd.Series, periods: list[int] = [1, 5, 10, 21]) -> pd.DataFrame:
    ...
    results = {}
    for p in periods:
        # log(p_t / p_{t-p}) captures compounding better than pct_change
...
def compute_rolling_volatility(returns_1d: pd.Series, windows: list[int] = [5, 10, 21, 63]) -> pd.DataFrame:
    """annualized rolling volatility at different horizons."""
    results = {}
    for w in windows:
        # annualize by sqrt(252 trading days)
...
def compute_rsi(series: pd.Series, period: int = 14) -> pd.Series:
    """relative strength index, classic wilder style."""
    delta = series.diff()
    ...
    # avoid division by zero when there are no losses in the window
...
def compute_atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14) -> pd.Series:
    """average true range, accounts for gaps via previous close."""
    prev_close = close.shift(1)
    # true range is the max of these three spans (handles overnight gaps)
    tr1 = high - low
...
def compute_volume_features(volume: pd.Series) -> pd.DataFrame:
    """volume ratio and momentum relative to 20d average."""
    results = {}
    vol_ma_20 = volume.rolling(window=20, min_periods=20).mean()
    # ratio > 1 means unusually high volume (potential breakout/event)
...
def compute_rate_of_change(series: pd.Series, period: int = 10) -> pd.Series:
    """simple momentum as percent change over n periods."""
...
def compute_bollinger_position(close: pd.Series, window: int = 20, num_std: float = 2.0) -> pd.Series:
    """where price sits within bollinger bands, 0 = lower band, 1 = upper."""
    sma = close.rolling(window=window, min_periods=window).mean()
    std = close.rolling(window=window, min_periods=window).std()
    upper = sma + num_std * std
    lower = sma - num_std * std
    width = upper - lower
    # clip to [0,1] so extreme readings don't blow up downstream
...
def engineer_features_for_instrument(
    ...
    """orchestrates all feature computation for a single instrument."""
    ...
    # realized vol needs 1d returns as input
    ...
    # parkinson vol only works if we have intraday range data
    if high is not None and low is not None:
        features["parkinson_vol_21d"] = compute_parkinson_volatility(high, low, window=21)
        print(f"    Parkinson vol: parkinson_vol_21d")

    # only keep distance features, raw smas aren't useful as model inputs
    ...
    # two momentum horizons to capture short and medium term trends
    ...
    # atr_norm scales by price so it's comparable across instruments
    if high is not None and low is not None:
    ...
    # gold as a risk-off signal relative to the instrument
    if "gld" in universe.columns:
        features["gold_equity_ratio"] = universe["gld"] / close
        print(f"    Gold: gold_equity_ratio")

    # relative strength vs spy (skip if we're already looking at spy)
    ...
    # sector rotation signals via sector/spy ratios
    ...
    # macro features come from fred, already in universe
    macro_cols = [
    ...
    # fama french factors for risk attribution
    ...
    # longer-horizon mean returns smooth out noise for regime detection
    if "log_ret_1d" in features.columns:
    ...
    # credit spread (hyg vs ief) is a leading risk indicator
    if "hyg" in universe.columns and "ief" in universe.columns:
        ...
        # zscore flags when credit conditions are abnormal vs recent history
    ...
    # dollar strength affects equities and crypto differently
    ...
    # forward fill lagged columns so we don't lose recent rows to NaN
    # (macro and ff data publishes with a delay)
...
def main():
    """entry point: loads universe, engineers features, saves per instrument and combined."""
    ...
    for symbol in TARGET_INSTRUMENTS:
        ...
        # tag rows so we can filter after combining all instruments
    ...
    # multi-index parquet so downstream can load everything at once
    if all_features:
